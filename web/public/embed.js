 ...
 * the HomeGPTagentChatbotConfig should be defined in the html file before this script is included
 * the HomeGPTagentChatbotConfig should contain the token of the chatbot
...
async function embedChatbot () {
  const HomeGPTagentChatbotConfig = window.HomeGPTagentChatbotConfig;
  if (!HomeGPTagentChatbotConfig || !HomeGPTagentChatbotConfig.token) {
    console.error('HomeGPTagentChatbotConfig is empty or token is not provided')
    return;
  }
  const isDev = !!HomeGPTagentChatbotConfig.isDev
  const baseUrl = HomeGPTagentChatbotConfig.baseUrl || `https://${isDev ? 'dev.' : ''}uHomeGPTagent.app`
  const openIcon = `<svg
            id="openIcon"
            width="24"
            height="24"
  ...
  function createIframe () {
    ...
    iframe.title = "HomeGPTagent chatbot bubble window"
    iframe.id = 'HomeGPTagent-chatbot-bubble-window'
    iframe.src = `${baseUrl}/chatbot/${HomeGPTagentChatbotConfig.token}`
  ...
  const targetButton = document.getElementById('HomeGPTagent-chatbot-bubble-button')
  if (!targetButton) {
    // create button
    const containerDiv = document.createElement("div");
    containerDiv.id = 'HomeGPTagent-chatbot-bubble-button'
    ...
    containerDiv.addEventListener('click', function () {
      const targetIframe = document.getElementById('HomeGPTagent-chatbot-bubble-window')
      if (!targetIframe) {
        createIframe()
