const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'HomeGPT 账号',
    langGeniusAccountTip: '您的 HomeGPT 账号和相关的用户数据。',
    editName: '编辑名字',
  ...
  members: {
    ...
    email: '邮箱',
    emailInvalid: '邮箱格式无效',
    jobNumberErr: '请输入有效的工号',
    emailPlaceholder: '输入邮箱',
    sendInvite: '发送邀请',
    invitedAsRole: '邀请为{{role}}用户',
    invitationSent: '邀请已发送',
    invitationSentTip: '邀请已发送，对方登录 HomeGPT 后即可访问你的团队数据。',
    invitationLink: '邀请链接',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '体验',
      exhausted: '超出限额',
      desc: '托管 OpenAI 由 HomeGPT 提供的托管 OpenAI 服务，你可以使用 GPT-3.5 等模型，在体验额度消耗完毕前你需要设置其它模型供应商。',
      callTimes: '调用次数',
  ...
  apiBasedExtension: {
    title: 'API 扩展提供了一个集中式的 API 管理，在此统一添加 API 配置后，方便在 HomeGPT 上的各类应用中直接使用。',
    link: '了解如何开发您自己的 API 扩展。',
    linkUrl: 'https://docs.HomeGPTagent.ai/v/zh-hans/advanced/api_based_extension',
    add: '新增 API 扩展',
    selector: {
      title: 'API 扩展',
  ...
  about: {
    changeLog: '更新日志',
    updateNow: '现在更新',
    nowAvailable: 'HomeGPT {{version}} 现已可用。',
    latestAvailable: 'HomeGPT {{version}} 已是最新版本。',
  },
  appMenus: {
    overview: '概览',
    promptEng: '编排',
